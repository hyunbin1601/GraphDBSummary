...
from neo4j_driver import driver as aura_driver

# --- 설정 변수 ---
MERGED_SUMMARY_FILE = "merged_summaries.json"

# --- Neo4j 연결 정보 (Aura) ---
# 공통 드라이버(neo4j_driver.py)를 사용하고, 데이터베이스명은 환경변수로 조정 가능
...
def main():
    ...
    try:
        ...
        print(
            f"✅ '{MERGED_SUMMARY_FILE}' 파일 로드 성공. 총 {len(all_summaries)}개의 요약 정보를 찾았습니다."
        )
    except FileNotFoundError:
        print(
            f"❌ 오류: '{os.path.abspath(MERGED_SUMMARY_FILE)}' 파일을 찾을 수 없습니다."
        )
        return
    except json.JSONDecodeError:
        print(
            f"❌ 오류: '{MERGED_SUMMARY_FILE}' 파일이 올바른 JSON 형식이 아닙니다 (BOM 또는 중간 중단 여부 확인)."
        )
        return
        ...
        if isinstance(all_summaries, list):
            ...
            for summary_data in tqdm(all_summaries, desc="Enriching DB with Summaries"):
                if summary_data and "error" not in summary_data:
                    model_llm_summary_in_db(aura_driver, summary_data)
        # [수정] JSON 파일이 딕셔너리 형식일 경우를 처리
        elif isinstance(all_summaries, dict):
            print(f"총 {len(all_summaries)}개의 요약 정보를 DB에 추가합니다...")
            for trace_id, summary_data in tqdm(
                all_summaries.items(), desc="Enriching DB with Summaries"
            ):
                if summary_data and "error" not in summary_data:
