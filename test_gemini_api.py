"""
Gemini API 응답 테스트 스크립트
여러 Gemini 모델의 API 응답을 테스트할 수 있습니다.
최신 Gemini API (2025) 문서를 기반으로 작성되었습니다.
"""
...
if not gemini_key:
    ...
    print("💡 .env 파일에 GEMINI_API_KEY=your-key-here 추가하세요.")
    print(
        "💡 API 키는 Google AI Studio (https://makersuite.google.com/app/apikey)에서 발급받을 수 있습니다."
    )
    exit(1)

print(f"✅ Gemini API 키 확인됨: {gemini_key[:10]}...\n")
...
def test_simple_prompt(model_name: str):
    ...
    try:
        ...
        llm = ChatGoogleGenerativeAI(
            model=model_name,
            google_api_key=gemini_key,  # API 키 명시적 전달
            temperature=0.1,
...
def main():
    ...
    print(f"\n💾 상세 결과가 gemini_api_test_results.json 파일에 저장되었습니다.")
